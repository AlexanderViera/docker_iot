...
env = Env()
env.read_env() #aca leo el archivo .env para cargar las variables de entorno
...
async def main():
    ...
    topico1 = env('TOPICO1')
    topico2 = env('TOPICO2')
    topico_pub = env('TOPICO_PUB')

    estado = Estado()
    cola_topico1 = asyncio.Queue()
    cola_topico2 = asyncio.Queue()
    ...
    async with aiomqtt.Client(
        ...
        async with asyncio.TaskGroup() as tg:
            ...
            tg.create_task(publicar(client, topico_pub, estado))
            tg.create_task(atender(cola_topico1, "AtencionTopic1"))
            tg.create_task(atender(cola_topico2, "AtencionTopic2"))
            tg.create_task(escuchar(client, topico1, topico2, cola_topico1, cola_topico2))
